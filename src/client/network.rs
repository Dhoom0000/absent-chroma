...
impl NetworkPlugin {
    ...
    fn receive_server_message(mut client: ResMut<RenetClient>, mut kem: ResMut<KEMClientKey>) {
            ...
            while let Some(message) = client.receive_message(channel_id) {
                ...
                match server_message {
                    ...
                    ServerMessage::KEMEncapsKey(encaps_key) => {
                        Self::respond_kem_handshake(*encaps_key, &mut client, &mut kem); // if its encaps key, then use the encryption logic to send ciphertext key
                    }
    ...
    fn respond_kem_handshake(
        encaps_key: [u8; 800],
        client: &mut RenetClient,
        kem: &mut KEMClientKey,
    ) {
